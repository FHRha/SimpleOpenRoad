...
| [Google OAuth for Gemini Code Assist](https://google-gemini.github.io/gemini-cli/docs/get-started/authentication.html) | Experimental Google OAuth provider for Gemini Code Assist / AI Pro subscriptions. Connect with `sor providers connect google`; the wizard uses direct PKCE OAuth with browser callback or manual code fallback, works with the built-in client by default, and `sor providers accounts` shows saved profiles. |
