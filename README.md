...
| [Gemini CLI OAuth](https://google-gemini.github.io/gemini-cli/docs/get-started/authentication.html) | Experimental Gemini CLI-style OAuth provider for Google AI Pro / Ultra subscriptions. Sign in with the official Gemini CLI (`GEMINI_FORCE_FILE_STORAGE=true gemini` on VPS), then connect with `sor providers connect google`. |
